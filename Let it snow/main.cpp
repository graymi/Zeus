                   ...
                   int nCmdShow)
{
	...
	mats->matProjection = matProjection;
	mats->matView = matView;
	...
	//A cow
	geometry->LoadObject( dev, devcon, "frog.obj", D3DXVECTOR4(0.1f,0.8f,0.3f,1.0f) );
...
//Create the master particle
	...
	for(int i = 3; i < MAXPARTICLES; i++)
	{
		...
		// Physics Init
		geometry->objects[i]->x0 = Ranf(-4.0f, 4.0f);
        geometry->objects[i]->y0 = 3.0f;
        geometry->objects[i]->z0 = Ranf(-4.0f, 4.0f);
        ...
        geometry->objects[i]->vx0 = Ranf(-0.0015f, 0.0015f);
        geometry->objects[i]->vy0 = Ranf(-0.35f, -0.085f);
        geometry->objects[i]->vz0 = Ranf(-0.0015f, 0.0015f);
	...
	geometry->SetNormal(dev,L"bumpnormal.png",MAXPARTICLES);
	geometry->SetAlpha(dev, L"snowalphasmallest.png", MAXPARTICLES);
	geometry->SetMapping(0.,1.,0.,0.,MAXPARTICLES);
    ...
    while(TRUE)
    {
		...
		D3DXMatrixRotationY(&matRotateX, D3DX_PI);
		D3DXMatrixTranslation(&matTrans, 2.0f, (sinf(Time * 10.0f)/4.0f) - 0.6f, 0.0f);
		...
		for(int i = 3; i < MAXPARTICLES; i++)
		{
			...
			if(geometry->objects[i]->y < -1.0f)
			{
				...
				geometry->objects[i]->x = geometry->objects[i]->x0 + geometry->objects[i]->vx * Time;
				geometry->objects[i]->y = (fmodf(geometry->objects[i]->y0 + geometry->objects[i]->vy * Time - 4.0, 4.0)) + 3.0f;
				geometry->objects[i]->z = geometry->objects[i]->z0 + geometry->objects[i]->vz * Time;
			}
